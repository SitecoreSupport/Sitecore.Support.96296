namespace Sitecore.Support.SessionProvider.Sql
{
  ...
  public class SqlSessionStateProvider : SitecoreSessionStateStoreProvider
  {
    ...
    private static readonly List<SqlSessionStateProvider> SqlSessionStateProvidersList =
      new List<SqlSessionStateProvider>();
      ...
      out TimeSpan lockAge, out object lockId, out SessionStateActions actions)
    {
      ...
      SessionStateStoreData result =
        this.Store.GetItemExclusive(this.ApplicationId, id, acquiredLockCookie, out existingLockCookie, out flags);
    ...
    public override void SetAndReleaseItemExclusive([NotNull] HttpContext context, string id,
      SessionStateStoreData sessionState, object lockId, bool newItem)
    {
      Assert.ArgumentNotNull(context, "context");
      Assert.ArgumentNotNull(id, "id");
    ...
    protected override SessionStateStoreData GetExpiredItemExclusive(DateTime signalTime,
      SessionStateLockCookie lockCookie, out string id)
    {
